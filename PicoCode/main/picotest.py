...
#  - User preview: Green trail; capture => destination Cyan
#  - Engine/Hint capture: destination Cyan
...
BUTTON_PINS = [2, 3, 4, 6, 7, 8, 9, 10, 12, 13]   # 1–8 coords, 9 OK, 10 Hint IRQ
DEBOUNCE_MS = 300
...
ENGINE_COLOR = BLUE  # deep blue for engine/computer moves
...
persistent_trail_end_color = None # None or CYAN
...
class Chessboard:
    ...
    # Upright plotting helper (visual top-left -> physical flip Y)
    ...
    def show_strength_prompt(self):
        ...
        self._plot_points_upright(T, MAGENTA)
        self.write()

    def show_time_prompt(self):
        self.clear(BLACK)
        L = [(2,1),(2,2),(2,3),(2,4),(2,5), (3,5),(4,5),(5,5)]
        self._plot_points_upright(L, MAGENTA)
        self.write()
...
# Legality/Capture verdict query
# Pico -> heypichk_<uci>
# Pi   -> heyArduinocheck_illegal_<uci>
#      -> heyArduinocheck_ok_<uci>_(cap|nocap)
...
def process_hint_irq():
    ...
    # New Game if OK held during hint
    if BTN_OK.value() == 0:
...
def enter_from_square(seed_btn=None):
    ...
    # If overlay active, clear it on first press (OK or coord)
...
def enter_to_square(move_from):
    ...
    # If overlay active, clear on first press
    if persistent_trail_active:
        while True:
            msg = read_from_pi()
            if msg:
...
def _color_for_user_confirm():
    # Local: keep GREEN for both sides per your last preference
    if game_mode == "local":
        return GREEN
    return GREEN
...
def select_game_mode():
    ...
    while True:
        b = buttons.detect_press()
        if b == 1:
            game_mode = "pc";     send_to_pi("btn_mode_pc");     return
        if b == 2:
            game_mode = "online"; send_to_pi("btn_mode_online"); return
        if b == 3:
            game_mode = "local";  send_to_pi("btn_mode_local");  return
        time.sleep_ms(5)
...
CYAN=(0,255,255); MAGENTA=(255,0,255); YELLOW=(255,255,0); ORANGE=(255,130,0)
ENGINE_COLOR = BLUE  # deep blue for engine/computer moves
CAPTURE_END_COLOR = MAGENTA  # requested capture destination color ("purple")
...
persistent_trail_type   = None    # 'hint' or 'engine'
persistent_trail_move   = None    # e.g., 'e2e4'
persistent_trail_end_color = None # None or CAPTURE_END_COLOR
...
# Legality/Capture verdict query (timeout 1500ms)
# Pico -> heypichk_<uci>
# Pi   -> heyArduinocheck_illegal_<uci>
#      -> heyArduinocheck_ok_<uci>_(cap|nocap)
...
def query_move_verdict(uci, timeout_ms=1500):
    send_to_pi("chk_", uci)
    time.sleep_ms(2)  # avoid UART contention
    t0 = time.ticks_ms()
    while time.ticks_diff(time.ticks_ms(), t0) < timeout_ms:
        msg = read_from_pi()
        if not msg:
    ...
    if persistent_trail_active:
        while True:
            ...
            if msg:
                if msg.startswith("heyArduinohint_"):
                    best = msg[15:].strip()
                    verdict, is_cap = query_move_verdict(best)
                    show_persistent_trail(best, YELLOW, 'hint', end_color=(CAPTURE_END_COLOR if (verdict=='legal' and is_cap) else None)); continue
                if msg.startswith("heyArduinom"):
                    mv = msg[11:].strip()
                    verdict, is_cap = query_move_verdict(mv)
                    show_persistent_trail(mv, ENGINE_COLOR, 'engine', end_color=(CAPTURE_END_COLOR if (verdict=='legal' and is_cap) else None)); continue
            b = buttons.detect_press()
            if not b:
                time.sleep_ms(5); continue
            clear_persistent_trail()
        ...
        if seed_btn is not None:
            ...
            if msg:
                ...
                if msg.startswith("heyArduinom"):
                    mv = msg[11:].strip()
                    verdict, is_cap = query_move_verdict(mv)
                    show_persistent_trail(mv, ENGINE_COLOR, 'engine', end_color=(CAPTURE_END_COLOR if (verdict=='legal' and is_cap) else None))
                    cancel_user_input_and_restart(); return None
            b = buttons.detect_press()
            if not b:
                time.sleep_ms(5); continue
...
def enter_to_square(move_from):
    ...
    board.show_markings()
    if is_cap:
        board.draw_trail(move_from + to, GREEN, end_color=CAPTURE_END_COLOR)
    else:
        board.draw_trail(move_from + to, GREEN)
    return to
...
def main_loop():
    ...
    while True:
        ...
        # ENGINE move -> persistent deep blue; capture end colored
        if msg.startswith("heyArduinom"):
            mv = msg[11:].strip()
            verdict, is_cap = query_move_verdict(mv)
            show_persistent_trail(mv, ENGINE_COLOR, 'engine', end_color=(CAPTURE_END_COLOR if (verdict=='legal' and is_cap) else None))
        ...
        # HINT -> persistent yellow; capture end colored
        if msg.startswith("heyArduinohint_"):
            best = msg[len("heyArduinohint_"):].strip()
            verdict, is_cap = query_move_verdict(best)
            show_persistent_trail(best, YELLOW, 'hint', end_color=(CAPTURE_END_COLOR if (verdict=='legal' and is_cap) else None))
