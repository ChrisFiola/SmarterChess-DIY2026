...
class ChessBoard:
    ...
    def getValidKingMoves(self, fromPos):
        ...
        if c_king:
            if (
                self.isFree(5, c_row)
                and self.isFree(6, c_row)
                and self._board[c_row][7].upper() == "R"
            ):
                if (
                    not self.isThreatened(4, c_row)
                    and not self.isThreatened(5, c_row)
                    and not self.isThreatened(6, c_row)
                ):
        ...
        if c_queen:
            if (
                self.isFree(3, c_row)
                and self.isFree(2, c_row)
                and self.isFree(1, c_row)
                and self._board[c_row][0].upper() == "R"
            ):
                if (
                    not self.isThreatened(4, c_row)
                    and not self.isThreatened(3, c_row)
                    and not self.isThreatened(2, c_row)
                ):
    ...
    def getFEN(self):
        ...
        if not (x == 0 and y == 0):
            if turn == "b" and (
                self._board[y][x - 1] == "p" or self._board[y][x + 1] == "p"
            ):
