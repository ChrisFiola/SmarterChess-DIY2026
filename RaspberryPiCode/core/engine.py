...
class EngineContext:
    ...
    def bestmove(self, brd: "chess.Board", ms: int) -> Optional[str]:
        if brd.is_game_over():
            return None

        chess_engine = self._engine_mod()
        engine = self.ensure(STOCKFISH_PATH)
        limit = chess_engine.Limit(time=max(0.01, ms / 1000.0))

        result = engine.play(brd, limit)
        return result.move.uci() if result.move else None

    def hint(self, brd: "chess.Board", ms: int) -> Optional[str]:
        if brd.is_game_over():
            return None
